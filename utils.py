...
from conlleval import return_report#导入对模型好坏的评判标准
...
def clean(params):
    ...
    if os.path.isdir(params.ckpt_path):
        shutil.rmtree(params.ckpt_path)#递归的去删除文件
...
def create_model(session, Model_class,load_vec, config, id_to_char):
    # create model, reuse parameters if exists
    model = Model_class(config)
    session.run(tf.global_variables_initializer())
    return model

def create_testmodel(session, Model_class, path, load_vec, config, id_to_char):#, logger
    model = Model_class(config)
    ckpt = tf.train.get_checkpoint_state(path)
    model.saver.restore(session, r"./ckpt")
    return model
